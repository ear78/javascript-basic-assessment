...
function coderTest(obj) {
	for (var key in obj) {
		if ( key === obj.name["Jeremey"]) {
			obj.lovesCode = 10;
		}
		else if (key === obj.name["Brack"]){
			obj.lovesCode = 0;
		}
		else {
			obj.lovesCode = 5;
...
function callerBack(string, callBack){
	return callBack;
}

function holla(string){
	return "Give it " + string;
}
